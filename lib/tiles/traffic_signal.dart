...
class TrafficSignalController {
  ...
  // approaching player meets a fresh phase within a run-up.
  static const double greenSeconds = 7.0;
  static const double yellowSeconds = 2.5;
  static const double allRedSeconds = 1.5;

  /// One group's slot: green → yellow → red (the red includes the shared all-red
  /// clearance with the other group).
  double get _slot => greenSeconds + yellowSeconds + allRedSeconds;
  double get _cycle => _slot * 2;
  ...
  /// THROUGH phase for an approach group. The N–S group leads the cycle; the E–W
  /// group is offset by exactly half a cycle, so whenever one group is in its
